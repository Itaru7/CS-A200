...
void selectionSort(int myArray[], int &count, int numOfElement);


// Declaration function minLocation
// Running time = O(n)
int minLocation(int myArray[], int first, int last, int& count);


// Declaration function insertionSort
// Running time: O(n^2) 
void insertionSort(int myArray[], int &count, int numOfElement);


// Declaration function mergeSort
// Running time: O(n log n) => this includes the call to function merge
// Uses function merge
void mergeSort(int myArray[], int &count, int numOfElement);


// Declaration function merge
// Running time: O(n)
void merge(int myArray[], int &count, int numOfElement);
...
void bubbleSort(int myArray[], int &count, int numOfElement)
{
	bool done = true;
	int j = 0;
	while (done && j < numOfElement - 1)
	{
		done = false;
		for (int i = 0; i < (numOfElement - 1 - j); i++)
		{
...
{
	int loc, minIndex;
	for (loc = 0; loc < numOfElement - 1; loc++)
	{
		minIndex = minLocation(myArray, loc, numOfElement - 1, count);
		swap(myArray[loc], myArray[minIndex]);
	}
}
...
int minLocation(int myArray[], int first, int last, int &count)
{
	...
	{
		if (myArray[loc] < myArray[minIndex])
			minIndex = loc;
		count++;
	}
	return minIndex;
